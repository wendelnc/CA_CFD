...
def eigenvalues(q_sys):
    ...
    caz = np.abs(np.sqrt(all_Bz**2 / all_rho))

    # 4) Compute Fast Magnetosonic Wave Speed
    cfz = np.sqrt(0.5 * np.abs( a**2 + ca**2 + np.sqrt((a**2 + ca**2)**2 - (4 * a**2 * caz**2))))

    # 5) Compute Slow Magnetosonic Wave Speed
    csz = np.sqrt(0.5 * np.abs( a**2 + ca**2 - np.sqrt((a**2 + ca**2)**2 - (4 * a**2 * caz**2))))

    # 3) Find Maximum Wave Speed
    max_eigen_1_z = max(em, np.max(np.abs(all_vez      )))
    max_eigen_2_z = max(em, np.max(np.abs(all_vez      )))
    max_eigen_3_z = max(em, np.max(np.abs(all_vez + caz)))
    max_eigen_4_z = max(em, np.max(np.abs(all_vez - caz)))
    max_eigen_5_z = max(em, np.max(np.abs(all_vez + cfz)))
    max_eigen_6_z = max(em, np.max(np.abs(all_vez - cfz)))
    max_eigen_7_z = max(em, np.max(np.abs(all_vez + csz)))
    max_eigen_8_z = max(em, np.max(np.abs(all_vez - csz)))

    alphaz = np.array([max_eigen_1_z, max_eigen_2_z, max_eigen_3_z, max_eigen_4_z, max_eigen_5_z, max_eigen_6_z, max_eigen_7_z, max_eigen_8_z])
