# Standard Python Libraries
import numpy as np
from numba import njit

# User Defined Libraries
import configuration as cfg      # Input Parameters
import cons2prim as c2p          # Convert Conserved to Primitive Variables


def save_q_sys(q_sys,t):

    rho, vex, vey, vez, pre, Bx, By, Bz = c2p.cons2prim(q_sys)

    format_spec = "{:13.16e} "

    filename = f"results/q_sys_at_time_{t}_for_{cfg.nx1}_by_{cfg.nx2}_by_{cfg.nx3}.txt"
    
    with open(filename, "w") as file:
        for i in range(cfg.nghost,(cfg.nx1)+cfg.nghost):
            for j in range(cfg.nghost,(cfg.nx2)+cfg.nghost):
                for k in range(cfg.nghost,(cfg.nx3)+cfg.nghost):
                    file.write(format_spec.format(rho[i,j,k]))
                    file.write(format_spec.format(vex[i,j,k]))
                    file.write(format_spec.format(vey[i,j,k]))
                    file.write(format_spec.format(vez[i,j,k]))
                    file.write(format_spec.format(pre[i,j,k]))
                    file.write(format_spec.format(Bx[i,j,k]))
                    file.write(format_spec.format(By[i,j,k]))
                    file.write(format_spec.format(Bz[i,j,k]))
                    file.write("\n")
    
    print("File created successfully.")

def save_a_sys(a_sys,t):

    format_spec = "{:13.16e}"

    filename = f"results/a_sys_at_time_{t}_for_{cfg.nx1}_by_{cfg.nx2}_by_{cfg.nx3}.txt"
    with open(filename, "w") as file:
        for i in range(cfg.nghost,(cfg.nx1)+cfg.nghost):
            for j in range(cfg.nghost,(cfg.nx2)+cfg.nghost):
                for k in range(cfg.nghost,(cfg.nx3)+cfg.nghost):
                    file.write(format_spec.format(a_sys[0,i,j,k]))
                    file.write(format_spec.format(a_sys[1,i,j,k]))
                    file.write(format_spec.format(a_sys[2,i,j,k]))
                    file.write("\n")

    print("File created successfully.")

def save_div(div,t):

    format_spec = "{:13.16e}"

    filename = f"results/div_at_time_{t}_for_{cfg.nx1}_by_{cfg.nx2}_by_{cfg.nx3}.txt"
    with open(filename, "w") as file:
        for i in range(cfg.nx1):
            for j in range(cfg.nx2):
                for k in range(cfg.nx3):
                    file.write(format_spec.format(div[i,j,k]))
                    file.write("\n")

    print("File created successfully.")