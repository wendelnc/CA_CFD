...
def save_q_sys(q_sys,t):
    ...
    filename = f"results/txt_files/q_sys_at_time_{t}_for_{cfg.nx1}_by_{cfg.nx2}.txt"
...
def save_a_sys(a_sys,t):
    ...
    filename = f"results/txt_files/a_sys_at_time_{t}_for_{cfg.nx1}_by_{cfg.nx2}.txt"
    with open(filename, "w") as file:
        for i in range(cfg.nghost,(cfg.nx1)+cfg.nghost):
            for j in range(cfg.nghost,(cfg.nx2)+cfg.nghost):
...
def save_div(div,t):
    ...
    filename = f"results/txt_files/div_at_time_{t}_for_{cfg.nx1}_by_{cfg.nx2}.txt"
    with open(filename, "w") as file:
